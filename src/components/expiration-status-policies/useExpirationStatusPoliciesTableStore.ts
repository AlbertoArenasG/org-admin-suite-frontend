...
type FiltersUpdater =
  ...
  | ((prev: ExpirationStatusPolicyListFilters) => ExpirationStatusPolicyListFilters);

interface ExpirationStatusPoliciesTableStoreState {
  ...
  deleteTarget: ExpirationStatusPoliciesTableRow | null;
  initialized: boolean;
}

interface ExpirationStatusPoliciesTableStoreActions {
  ...
  setDeleteTarget: (policy: ExpirationStatusPoliciesTableRow | null) => void;
  syncFromUrl: (state: {
    pagination: PaginationState;
    sorting: SortingState;
    globalFilter: string;
...
export type ExpirationStatusPoliciesTableStore = ExpirationStatusPoliciesTableStoreState &
  ExpirationStatusPoliciesTableStoreActions;
...
function applyFiltersUpdater(
  ...
  current: ExpirationStatusPolicyListFilters
): ExpirationStatusPolicyListFilters {
  return typeof updater === 'function' ? updater(current) : updater;
}

function createInitialState(): ExpirationStatusPoliciesTableStoreState {
  return {
...
function areFiltersEqual(
  a: ExpirationStatusPolicyListFilters,
  b: ExpirationStatusPolicyListFilters
) {
  return a.status === b.status;
}

export const useExpirationStatusPoliciesTableStore = create<ExpirationStatusPoliciesTableStore>(
  (set) => ({
    ...createInitialState(),
    ...
    syncFromUrl: (nextState) => {
      ...
      set((state) => {
        const updates: Partial<ExpirationStatusPoliciesTableStoreState> = {};
