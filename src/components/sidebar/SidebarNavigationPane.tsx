              ...
              >
                <CollapsibleTrigger asChild>
                  <button
                    type="button"
                    className={cn(
                      'flex min-h-10 w-full min-w-0 items-center gap-2 rounded-lg px-3 text-left text-sm text-sidebar-foreground/85 outline-hidden transition-colors hover:bg-white/10 hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring',
                      entry.isActive && 'bg-white/10 text-sidebar-foreground'
                    )}
                  >
                    {entry.icon ? <entry.icon className="size-4 shrink-0" /> : null}
                    <span className="min-w-0 flex-1 truncate">{entry.title}</span>
                    <ChevronRight className="size-4 shrink-0 transition-transform duration-200 group-data-[state=open]/entry:rotate-90" />
                  </button>
                </CollapsibleTrigger>
                <CollapsibleContent className="sidebar-nav-collapsible-content">
                  <ul className="border-sidebar-border/80 mt-1 ml-5 flex min-w-0 flex-col gap-1 border-l py-0.5 pl-3">
...
function SidebarNavigationLink({ entry, onNavigate, root = false }: SidebarNavigationLinkProps) {
  ...
  return (
    <Link
      ...
      onClick={onNavigate}
      className={cn(
        'flex min-h-9 min-w-0 items-center gap-2 rounded-lg px-3 text-sm text-sidebar-foreground/75 outline-hidden transition-colors hover:bg-white/10 hover:text-sidebar-foreground focus-visible:ring-2 focus-visible:ring-sidebar-ring',
        root && 'min-h-10 text-sidebar-foreground/85',
