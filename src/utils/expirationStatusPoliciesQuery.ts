...
const SORT_COLUMN_FIELD_MAP: Record<string, ExpirationStatusPolicySortField> = {
  name: 'name',
  status: 'status',
...
export function mapExpirationStatusPolicySortingToApi(
  ...
  return sorting
    .map((sort) => {
      ...
      } as const satisfies ExpirationStatusPolicyListSort;
    })
    .filter(Boolean) as ExpirationStatusPolicyListSort[];
}

export function parseExpirationStatusPolicySortingFromParams(
...
export function buildExpirationStatusPoliciesQuery(params: {
  pageIndex: number;
  pageSize: number;
  search: string;
  sorting: SortingState;
  ...
  mapExpirationStatusPolicySortingToApi(params.sorting).forEach((sort, index) => {
