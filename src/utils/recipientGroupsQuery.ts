...
const SORT_COLUMN_FIELD_MAP: Record<string, RecipientGroupSortField> = {
  name: 'name',
  status: 'status',
...
export function mapRecipientGroupSortingToApi(sorting: SortingState): RecipientGroupListSort[] {
  return sorting
    .map((sort) => {
      ...
      } as const satisfies RecipientGroupListSort;
    })
    .filter(Boolean) as RecipientGroupListSort[];
}

export function parseRecipientGroupSortingFromParams(params: URLSearchParams): SortingState {
...
export function buildRecipientGroupsQuery(params: {
  pageIndex: number;
  pageSize: number;
  search: string;
  ...
  mapRecipientGroupSortingToApi(params.sorting).forEach((sort, index) => {
