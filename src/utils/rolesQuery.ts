...
export function mapRoleSortingToApi(sorting: SortingState): RoleListSort[] {
  return sorting
    .map((sort) => {
      ...
      return { field, direction: sort.desc ? 'desc' : 'asc' } as const satisfies RoleListSort;
    })
    .filter(Boolean) as RoleListSort[];
}

export function parseRoleSortingFromParams(params: URLSearchParams): SortingState {
...
export function buildRolesQuery(params: {
  pageIndex: number;
  pageSize: number;
  search: string;
  ...
  mapRoleSortingToApi(params.sorting).forEach((sort, index) => {
